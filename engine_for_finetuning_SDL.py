...
import torch
from utils_SDL.mixup import Mixup
...
def train_class_batch(model, samples, target, criterion):
    outputs, feat = model(samples)
    loss = criterion(outputs, target)
    return loss, outputs, feat

@torch.no_grad()
def update_class_quene(sdl, outputs, feat, target):
    sdl.update(feat, outputs, target)
...
def train_one_epoch(model: torch.nn.Module, sdl,
                    criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                    model_ema: Optional[ModelEma] = None, mixup_fn: Optional[Mixup] = None, log_writer=None,
                    start_steps=None, lr_schedule_values=None, wd_schedule_values=None,
                    num_training_steps_per_epoch=None, update_freq=None, sdl_update_freq=None):
    model.train(True)
    
    sdl_loss_fn = criterion['sdl_loss_fn']
    criterion = criterion['criterion']
    
    sdl_loss_fn.update_weight(epoch) if sdl_loss_fn else None
    
    metric_logger = utils_SDL.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils_SDL.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('min_lr', utils_SDL.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        ori_samples = samples.to(device, non_blocking=True)
        ori_targets = targets.to(device, non_blocking=True)


        if mixup_fn is not None:
            samples, targets = mixup_fn(ori_samples, ori_targets)
        else:
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast():
                loss, output, feat = train_class_batch(
                    model, samples, targets, criterion)
        loss_value = loss.item()
        
        sdl_loss_vaule=-1.0
        if sdl_loss_fn and epoch > 20:
            soft_targets = sdl(feat, output, targets) # 计算 soft_targets
            sdl_loss = sdl_loss_fn(output, soft_targets)
            sdl_loss_vaule = sdl_loss.item()
            loss += sdl_loss
        ...
        metric_logger.update(sdl_loss=sdl_loss_vaule)
        ...
        if sdl_loss_fn and epoch%sdl_update_freq==0 and epoch >20:
            with torch.cuda.amp.autocast():
                with torch.no_grad():
                    outputs, feat = model(ori_samples)
                    update_class_quene(sdl,outputs,feat,ori_targets)

                
        if log_writer is not None:
            log_writer.update(loss=loss_value, head="loss")
            log_writer.update(loss=sdl_loss_vaule, head="sdl_loss")
...
def final_test(data_loader, model, device, file, save_feature=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            # else:
            output, saved_feature = model(videos)
            loss = criterion(output, target)
