...
from datasets_org import build_dataset
from engine_for_finetuning_SDL import train_one_epoch, validation_one_epoch, final_test, merge

from utils import NativeScalerWithGradNormCount as NativeScaler
from utils import  multiple_samples_collate
import utils

import utils_SDL.loss
...
def get_args():
    ...
    parser.add_argument('--pretrain_model', default='/home/gpuadmin/MB/FMAE_RAFDB.pth', help='finetune from checkpoint')
    parser.add_argument('--global_pool', action='store_true')
    parser.set_defaults(global_pool=True)
    parser.add_argument('--cls_token', action='store_false', dest='global_pool',
                        help='Use class token instead of global pool for classification')

    parser.add_argument('--min_area', default=0.02, type=float)
    parser.add_argument('--max_area', default=1 / 3, type=float)
    ...
    parser.add_argument('--clip_grad', type=float, default=None, metavar='NORM', #None
    ...
    parser.add_argument('--warmup_epochs', type=int, default=0, metavar='N',
                        help='epochs to warmup LR, if scheduler supports') #min에서 max로 올라가는데 걸리는 epoch 5
    parser.add_argument('--warmup_steps', type=int, default=-1, metavar='N', 
                        help='num of steps to warmup LR, will overload warmup_epochs if set > 0') #총 몇번 warm up할껀지
    ...
    parser.add_argument('--mixup', type=float, default=0.8,
                        help='mixup alpha, mixup enabled if > 0.') # 0.8 or -1
    ...
    #parser.add_argument('--finetune', default='', help='finetune from checkpoint')
    # parser.add_argument('--eval_valid', default=True, type=str,
    #                     help='eval by traditional methods')
    # parser.add_argument('--eval_test', default=False, type=str,
    #                     help='eval prediction over two uniformly sampled video clips')
...
def main(args, ds_init):
    ...
    if dataset_val is not None:
        data_loader_val = torch.utils.data.DataLoader(
            ...
            batch_size=int(3 * args.batch_size), # 3 
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False
        )
    ...
    mixup_fn = None
    mixup_active = False #or args.cutmix > 0. or args.cutmix_minmax is not None
    if args.mixup > 0:
        mixup_active = True 
    if mixup_active:
    ...
    sdl = SDL(args.nb_classes, k=2, size=16).to(device)


    ###loading for resume model
    if args.finetune:
        if os.path.isfile(args.finetune):
            ...
            #print(args.epoch)
            #print("=> loaded checkpoint '{}' (epoch {})".format(args.finetune))
        else:
    ...
    ###loading for evaluate model
    if args.eval:
        if os.path.isfile(args.eval):
                print("=> loading checkpoint '{}'".format(args.eval))
            ...
            print("=> no checkpoint found at '{}'".format(args.eval))
            exit()
    ...
    # args.lr = 1e-4 #args.lr * total_batch_size / 256 # 256 best is 8e-5
    # args.min_lr = 1e-7 # args.min_lr * total_batch_size / 256 # 256 8e-6
    # args.warmup_lr = 1e-7 #args.warmup_lr * total_batch_size / 256 # 256  8e-6
    print("LR = %.8f" % args.lr)
    ...
    sdl_loss_fn = utils_SDL.loss.BCEWithLogitsLoss()
    ...
    # if args.eval and args.eval_valid:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            num_training_steps_per_epoch=num_training_steps_per_epoch, update_freq=args.update_freq,sdl_update_freq=100
        )
...
#         if isinstance(dataset, VideoClsDatasetFrame):
#             return [x for x in dataset.label_array]
#         else:
