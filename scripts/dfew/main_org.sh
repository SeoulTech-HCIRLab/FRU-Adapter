...
min_lr=1e-4 #1e-4
epochs=100 # 300
# model_checkpoint="./saved/model/finetuning/${finetune_dataset}/DFEW_org/eval_split0${splits}/checkpoint-best.pth" #

for split in "${splits[@]}";
    do
      OUTPUT_DIR="./saved/model/finetuning/${finetune_dataset}/DFEW_org_train/eval_split0${split}"
      if [ ! -d "$OUTPUT_DIR" ]; then
        mkdir -p $OUTPUT_DIR
      fi
      # path to split files (train.csv/val.csv/test.csv)
      DATA_PATH="./saved/data/${finetune_dataset}/clip_224x224/split0${split}"
