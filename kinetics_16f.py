...
class VideoClsDatasetFrame(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        if (mode == 'train'):
            ...
            self.data_resize = video_transforms.Compose([
                ...
                # video_transforms.ColorJitter(brightness=0.5), #make 
            ])
            pass

        elif (mode == 'validation' or 'test'):
    ...
    def __getitem__(self, index):
        ...
        elif self.mode == 'validation' or 'test':
            sample = self.dataset_samples[index]
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        # buffer = vr.load(all_index)
        # return buffer
        all_index = [x for x in range(0, len(vr))] # 0 ~ 15
        buffer = vr.load(all_index)
        return buffer
...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        ...
        # frames = video_transforms.ColorJitter(brightness=0.5), #make @@@@@
    else:
