...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, grad_reverse=0, num_classes=7, num_subjects=0, **kwargs):
        ...
        self.tsea_blocks = nn.ModuleList([
            FRU_Adapter(embded_dim=1024) for _ in range(len(self.blocks))
        ])
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x) + self.tsea_blocks[i](x)
        if self.global_pool:
            x = x[:, 1:, :].mean(dim=1)  # enter this one and it is global pool without cls token  
