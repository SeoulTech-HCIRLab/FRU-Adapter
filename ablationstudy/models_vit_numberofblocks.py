...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, global_pool=False, grad_reverse=0, num_classes=7, num_subjects=0, **kwargs):
        ...
        self.number_of_TSEA = 2 
        self.tsea_blocks = nn.ModuleList([
            TemporalSqueezeExcitationAdapter(embded_dim=1024) for _ in range(self.number_of_TSEA)
        ])
    ...
    def forward_features(self, x):
        ...
        order_of_TSEA = [x.item() for x in torch.linspace(0, 23, self.number_of_TSEA,dtype=torch.uint8)]
        
        for i, blk in enumerate(self.blocks):
            if i in order_of_TSEA:
                x = blk(x) + self.tsea_blocks[order_of_TSEA.index(i)](x) # b 197 1024
            else:
                x = blk(x)
        if self.global_pool:
        ...
        #         x = blk(x) + self.tsea_blocks[i//block_interval](x) # b 197 1024
        #     else:
        #         x = blk(x)
        # if self.global_pool:
        #     x = x[:, 1:, :].mean(dim=1)  # global pool without cls token 여기 들어감 
