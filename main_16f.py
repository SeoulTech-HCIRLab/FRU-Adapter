...
from datasets_16f import build_dataset
from engine_for_finetuning import train_one_epoch, validation_one_epoch, final_test, merge
from engine_for_confusionmatrix import validation_one_epoch_confusion_matrix
...
# from kinetics_t import VideoClsDatasetFrame
from kinetics_16f import VideoClsDatasetFrame
...
def get_args():
    ...
    parser.add_argument('--mixup', type=float, default=-1,
                        help='mixup alpha, mixup enabled if > 0.') # 0.8
...
def main(args, ds_init):
    ...
    # if args.pretrain_model and not args.eval:
    ...
    if args.enable_deepspeed:
        ...
        if args.distributed:
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu],find_unused_parameters=True)#, find_unused_parameters=True
    ...
    #lr_schedule_values = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=5, eta_min=1e-5) # 300 50 250/5=50
    ...
    if args.eval:
        ...
        test_stats = validation_one_epoch_confusion_matrix(data_loader_val, model, device)
        exit(0)
